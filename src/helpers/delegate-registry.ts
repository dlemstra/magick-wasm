...
export class DelegateRegistry {
    ...
    private static _images: Record<string, IMagickImage> = {};
    ...
    static setProgressDelegate(image: IMagickImage): void {
        ...
        this._images[image._instance.toString()] = image;
        ImageMagick._api._MagickImage_SetClientData(image._instance, image._instance);
        ImageMagick._api._MagickImage_SetProgressDelegate(image._instance, _castToSize(DelegateRegistry._progressDelegate));
    }

    static removeProgressDelegate(image: IMagickImage): void {
        ImageMagick._api._MagickImage_SetClientData(image._instance, ImageMagick._api._NullPointer);
        ImageMagick._api._MagickImage_SetProgressDelegate(image._instance, ImageMagick._api._NullPointer);
        delete DelegateRegistry._images[image._instance.toString()];
    }
    ...
    private static logDelegate(eventType: number, messageInstance: number): void {
        ...
        const message = _createString(_castToSize(messageInstance), '');
    ...
    private static progressDelegate(originInstance: number, offsetBigInt: bigint, extentBigInt: bigint, userData: number): number {
        ...
        const origin = _createString(_castToSize(originInstance));
